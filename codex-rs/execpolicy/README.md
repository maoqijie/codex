...
- From the Codex CLI, run `codex2 execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
codex2 execpolicy check --rules path/to/policy.rules git status
```
