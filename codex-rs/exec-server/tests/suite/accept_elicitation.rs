...
fn ensure_codex_cli() -> Result<PathBuf> {
    ...
    let metadata = codex_cli.metadata().with_context(|| {
        format!(
            "failed to read metadata for codex2 binary at {}",
            codex_cli.display()
        )
    })?;
    ensure!(
        metadata.is_file(),
        "expected codex2 binary at {} to be a file; run `cargo build -p codex-cli --bin codex2` before this test",
        codex_cli.display()
    );
    ...
    let mode = metadata.permissions().mode();
        ...
        mode & 0o111 != 0,
        "codex2 binary at {} is not executable (mode {mode:o}); run `cargo build -p codex-cli --bin codex2` before this test",
        codex_cli.display()
    );
