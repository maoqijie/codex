...
async fn status_snapshot_shows_unlimited_credits() {
    ...
    assert!(
        rendered
            .iter()
            .any(|line| line.contains("积分:") && line.contains("不限")),
        "expected credits line, got {rendered:?}"
    );
}
...
async fn status_snapshot_shows_positive_credits() {
    ...
    assert!(
        rendered
            .iter()
            .any(|line| line.contains("积分:") && line.contains("13 积分")),
...
async fn status_snapshot_hides_zero_credits() {
    ...
    let rendered = render_lines(&composite.display_lines(120));
    assert!(
        rendered.iter().all(|line| !line.contains("积分:")),
