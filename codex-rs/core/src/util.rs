...
mod tests {
  ...
  "error": {
    ...
    fn resume_command_prefers_name_over_id() {
        ...
        assert_eq!(command, Some("codex2 resume my-thread".to_string()));
    }
    ...
    fn resume_command_quotes_thread_name_when_needed() {
        ...
        assert_eq!(
            command,
            Some("codex2 resume -- -starts-with-dash".to_string())
        );

        let command = resume_command(Some("two words"), None);
        assert_eq!(command, Some("codex2 resume 'two words'".to_string()));

        let command = resume_command(Some("quote'case"), None);
        assert_eq!(command, Some("codex2 resume \"quote'case\"".to_string()));
    }
}
