...
fn codex_command(codex_home: &Path) -> Result<assert_cmd::Command> {
    let mut cmd = assert_cmd::Command::new(codex_utils_cargo_bin::cargo_bin("codex2")?);
...
async fn list_and_get_render_expected_output() -> Result<()> {
    ...
    assert!(stdout.contains("docs"));
    assert!(stdout.contains("传输：stdio"));
    assert!(stdout.contains("命令：docs-server"));
    assert!(stdout.contains("参数：--port 4000"));
    assert!(stdout.contains("环境：TOKEN=*****"));
    assert!(stdout.contains("APP_TOKEN=*****"));
    assert!(stdout.contains("WORKSPACE_ID=*****"));
    assert!(stdout.contains("启用：true"));
    assert!(stdout.contains("删除：codex2 mcp remove docs"));
